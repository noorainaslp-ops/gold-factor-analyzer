"""
MULTI-FACTOR MARKET ENGINE V6.3.14
...
def evaluate_candidate(
    ...
    p = raw_probability(row, regime)
    er1, er3, er5 = expected_returns(row, regime)
    stop, target1, target2, rr1, rr2, risk = risk_levels(row)
    ...
    momentum_score = np.clip(50 + 1000 * row["mom_score"], 0, 100)
    volume_score = np.clip(50 + 35 * (row["vol_ratio"] - 1), 0, 100)
    rsi = float(row["rsi"])
    ...
    regime_score = {"FAVORABLE": 100, "MIXED": 60, "UNFAVORABLE": 30, "UNKNOWN": 40}.get(regime, 40)
    rr_score = np.clip(50 + 25 * (rr2 - 1), 0, 100)
    probability_score = 50 + 400 * (p - 0.50)
    expected_score = np.clip(50 + 500 * er3 + 250 * er5, 0, 100)

    composite = (
        0.27 * probability_score + 0.18 * expected_score
        + 0.15 * trend_score + 0.12 * momentum_score
        + 0.08 * volume_score + 0.08 * rsi_score
        + 0.07 * rr_score + 0.05 * regime_score
    )
    composite += 8 * (float(row["quality"]) - 0.50)
    ...
    if (
        composite >= 70 and p >= 0.56 and er3 > 0 and er5 > 0
        and rr1 >= 0.85 and rr2 >= 1.25 and row["vol_ratio"] >= 0.70
        and rsi <= 72 and row["trend20"] >= -0.002
    ):
        action = "TRADE"
    elif (
        composite >= 60 and p >= 0.53 and er3 > -0.001
        and rr2 >= 1.10 and row["vol_ratio"] >= 0.55
    ):
        action = "WATCH"
    else:
        action = "WAIT"
    ...
    failures = []
    if p < 0.56: failures.append("probability")
    if er3 <= 0: failures.append("er3")
    if er5 <= 0: failures.append("er5")
    if rr1 < 0.85: failures.append("rr1")
    if rr2 < 1.25: failures.append("rr2")
    if row["vol_ratio"] < 0.70: failures.append("volume")
    if rsi > 72: failures.append("rsi")
    if row["trend20"] < -0.002: failures.append("trend")

    return {
        "raw_probability": p, "er1": er1, "er3": er3, "er5": er5,
        "stop": stop, "target1": target1, "target2": target2,
        "rr1": rr1, "rr2": rr2, "risk": risk, "composite": composite,
        "action": action, "rsi": rsi, "volume": float(row["vol_ratio"]),
        "failures": failures,
    }
...
def main():
    ...
    lines.append(
        "V6.3.11 uses composite ranking, "
